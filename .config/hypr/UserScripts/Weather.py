...
TOOLTIP_MARKUP = os.getenv("WEATHER_TOOLTIP_MARKUP", "1").lower() not in (
    "0",
    "false",
    "no",
)
...
def build_hourly_precip(forecast: Dict[str, Any]) -> str:
    try:
        ...
        probs: List[Optional[float]] = (
            safe_get(forecast, "hourly", "precipitation_probability", default=[]) or []
        )
...
def build_output(
    ...
    temp_str = (
        f"{int(round(temp_val))}{temp_unit}"
    ...
    tmin_str = (
        f"{int(round(tmin_val))}{dtemp_unit}"
        if isinstance(tmin_val, (int, float))
        else ""
    )
    tmax_str = (
        f"{int(round(tmax_val))}{dtemp_unit}"
    ...
    wind_text = (
        f"  {int(round(wind_val))}{wind_unit}"
    ...
    visibility_text = (
        f"  {format_visibility(vis_val)}" if isinstance(vis_val, (int, float)) else ""
    )
...
def main() -> None:
    ...
    if cached and isinstance(cached, dict):
        ...
        aqi = cached.get("aqi")
        cached_place = (
            cached.get("place") if isinstance(cached.get("place"), str) else None
        )
    ...
    try:
        ...
        try:
            if os.path.exists(API_CACHE_PATH):
                with open(API_CACHE_PATH, "r", encoding="utf-8") as f:
                    stale = json.load(f)
                out, simple = build_output(
                    lat,
                    lon,
                    stale.get("forecast", {}),
                    stale.get("aqi"),
                    stale.get("place") if isinstance(stale.get("place"), str) else None,
                )
                print(json.dumps(out, ensure_ascii=False))
                write_simple_text_cache(simple)
                return
